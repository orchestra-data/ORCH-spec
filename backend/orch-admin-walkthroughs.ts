...
class OrchAdminWalkthroughs {
  ...
  async start(client: PoolClient, params: { walkthroughId: string; userId: string }): Promise<{ walkthroughId: string; steps: WalkthroughStep[] }> {
    await client.query(
    ...
    const { rows } = await client.query(
      `SELECT steps FROM orch_admin_walkthrough WHERE id = $1`,
      [params.walkthroughId]
    );

    const steps = typeof rows[0]?.steps === 'string' ? JSON.parse(rows[0].steps) : rows[0]?.steps ?? [];
    return { walkthroughId: params.walkthroughId, steps };
  }
  ...
  async complete(client: PoolClient, params: { walkthroughId: string; userId: string }): Promise<void> {
    await client.query(
      `UPDATE orch_admin_walkthrough_usage
       ...
       WHERE walkthrough_id = $1 AND user_id = $2 AND status = 'started'`,
      [params.walkthroughId, params.userId]
    );
